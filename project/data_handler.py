...
def preprocess_data(
    df, window=config.WINDOW, annualization_factor=config.ANNUALIZATION_FACTOR
):
    ...
    if len(df) < initial_rows:
        print(
            f"Dropped {initial_rows - len(df)} rows with missing essential\
                price/volume data."
        )
...
def split_data_by_shock(df, shock_date=config.SHOCK_DATE):
    ...
    print(
        f"Data split: {len(df_before_shock)} points before {shock_date},"
