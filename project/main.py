...
import modeling
import simulation
from evaluation import calculate_price_metrics, calculate_volatility_metrics, display_combined_metrics, qlike_loss
...
def run_analysis():
    ...
    if 'Log_Return' not in df.columns or actual_vol_col_name not in df.columns:
        print(f"Error: Essential columns ('Log_Return', '{actual_vol_col_name}') not found after preprocessing. Exiting.")
        return
    print(f"Using volatility column: '{actual_vol_col_name}'")

    print("\n--- 2. Initial Visualization ---")
    visualization.plot_initial_data(df, ticker=config.TICKER, shock_date=config.SHOCK_DATE, vol_col=actual_vol_col_name)
    ...
    if garch_11_model_results:
        ...
        sim_prices_garch_fixed, sim_vol_garch_fixed = simulation.run_garch_simulation(
            garch_11_model_results, df_after_shock, last_actual_price, n_sims=config.N_SIMS
        )
        if sim_prices_garch_fixed is not None:
            ...
            display_combined_metrics(garch_fixed_price_metrics, garch_fixed_vol_metrics, model_name_garch_fixed)
    ...
    if gjr_111_model_results:
        ...
        sim_prices_gjr_fixed, sim_vol_gjr_fixed = simulation.run_garch_simulation(
            gjr_111_model_results, df_after_shock, last_actual_price, n_sims=config.N_SIMS
        )
        if sim_prices_gjr_fixed is not None:
            ...
            display_combined_metrics(gjr_fixed_price_metrics, gjr_fixed_vol_metrics, model_name_gjr_fixed)
    ...
    sim_prices_garch_adapt, sim_vol_garch_adapt = simulation.run_adaptive_garch_simulation(
    ...
    if sim_prices_garch_adapt is not None:
        ...
        display_combined_metrics(garch_adapt_price_metrics, garch_adapt_vol_metrics, model_name_garch_adapt)
    ...
    if sim_prices_gjr_adapt is not None:
        ...
        display_combined_metrics(gjr_adapt_price_metrics, gjr_adapt_vol_metrics, model_name_gjr_adapt)
    ...
    if sim_prices_dyn is not None:
        ...
        display_combined_metrics(dyn_naive_price_metrics, dyn_naive_vol_metrics, approach_name_dyn)
    ...
    if sim_prices_fixed is not None:
        ...
        if pd.isna(constant_vol_val):
             print("Warning: Could not retrieve constant volatility for Fixed Naive model.")
             predicted_vol_fixed = pd.Series(np.nan, index=actual_vol_post_shock.index)
             fixed_naive_vol_metrics = {'RMSE_vol': np.nan, 'MAE_vol': np.nan, 'QLIKE_var': np.nan}
        else:
            predicted_vol_fixed = pd.Series(
                constant_vol_val,
                index=actual_vol_post_shock.index
            )
            fixed_naive_vol_metrics = calculate_volatility_metrics(
                actual_vol_post_shock, predicted_vol_fixed
            )

        display_combined_metrics(fixed_naive_price_metrics, fixed_naive_vol_metrics, approach_name_fixed)
        ...
        if not pd.isna(constant_vol_val):
            ...
            print(f"Skipping plot for {approach_name_fixed} due to missing volatility parameter.")
