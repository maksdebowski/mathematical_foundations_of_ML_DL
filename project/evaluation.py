...
def qlike_loss(actual, predicted):
    ...
    mask = ~np.isnan(actual) & ~np.isnan(predicted) & (actual > epsilon) & (predicted > epsilon)
    if np.sum(mask) == 0:
        return np.nan
...
def calculate_volatility_metrics(actual_volatility, predicted_volatility):
    ...
    combined = pd.concat([actual_volatility.rename('Actual_Vol'),
                          predicted_volatility.rename('Predicted_Vol')], axis=1).dropna()

    if combined.empty:
        print("Warning: No overlapping non-NaN data between actual and predicted volatility.")
    ...
    if common_index.empty:
        ...
        qlike_var = qlike_loss(actual_var.loc[common_index], predicted_var.loc[common_index])
...
def display_combined_metrics(price_metrics, vol_metrics, model_name="Model"):
    ...
    if price_metrics:
        for key, value in price_metrics.items():
            ...
            data.append({'Metric': metric_name, 'Type': metric_type.capitalize(), 'Value': value})
    if vol_metrics:
